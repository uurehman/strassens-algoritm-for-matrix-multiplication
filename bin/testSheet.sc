...
object testSheet {
  ...
  val orderPower: Int = 1
  val matrixOrder: Int = Math.pow(2, orderPower).intValue

  val matrixA = Array.fill(matrixOrder, matrixOrder)(scala.util.Random.nextInt(20))
  val matrixB = Array.fill(matrixOrder, matrixOrder)(scala.util.Random.nextInt(20))
  println("Printing Matrix A")
  printArray(matrixA)
  println("Printing Matrix B")
  printArray(matrixB)

  print(sMultiply(matrixA, matrixB))

  def printArray[Int](ArrayGiven: Array[Int]) = for (vElement <- ArrayGiven) println(vElement + "\t\t")
